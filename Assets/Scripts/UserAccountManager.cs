...
public class UserAccountManager : MonoBehaviour
{
    ...
    { //called when the 'Get Data' button on the data part is pressed
        ...
        {
            //ready to send request
            StartCoroutine(sendGetDataRequest(LoggedIn_Username, LoggedIn_Password, onDataReceived)); //calls function to send get data reques
        }
    }
    ...
    IEnumerator sendGetDataRequest(string username, string password, OnDataReceivedCallback onDataReceived)
    {
        ...
        {
            //Error occurred. For more information of the error, DC.Login could be used with the same username and password
            Debug.Log("Data Upload Error. Could be a server error. To check try again, if problem still occurs, contact us.");
        }
        else
        {
            ...
            else
            {
                ...
                string DataReceived = returnedddd.text;
                data = DataReceived;
            }
        }
